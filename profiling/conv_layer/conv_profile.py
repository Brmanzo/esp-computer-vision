...
_LUT4: dict[tuple, tuple] = {}   # (ib, wb, dsp) -> (A, B, C, D, R2)
...
def _lc(ic: int, oc: int, ib: int, wb: int, dsp: int) -> float:
    # DSP>0 corners are keyed by (ib, -1, dsp) — WB doesn't affect LUT4 in DSP mode
    key = (ib, -1, dsp) if dsp > 0 else (ib, wb, dsp)
    if key not in _LUT4:
        if dsp > 0:
            raise KeyError(
                f"No LUT4 model for InBits={ib} DSPCount={dsp} — "
                f"run fill_corner(ib={ib}, dsp={dsp}) to synthesize it"
            )
        raise KeyError(f"No LUT4 model for InBits={ib} WeightBits={wb} DSPCount={dsp}")
    A, B, C, D, _ = _LUT4[key]
...
def frontier(ib: int, wb: int, dsp: int = 0,
    ...
    [(ic, oc, predicted_lc, predicted_ff), ...] sorted by lut4 ascending.
    """
    results = []
    for ic in range(1, ic_max + 1):
        for oc in range(1, oc_max + 1):
            try:
                lut4, ff = predict_conv_layer(ic, oc, ib, wb, dsp)
                results.append((ic, oc, lut4, ff))
            except (ValueError, KeyError):
                break   # LUT4 monotone in OC for DSP=0; safe to break
...
def fill_corner(ib: int, dsp: int,
    ...
    """
    Synthesize a missing (ib, dsp) LUT4 corner, refit the regression, and reload.
...
def _report() -> None:
    ...
    dsp0 = [(ib, wb, dsp) for ib, wb, dsp in sorted(_LUT4) if dsp == 0]
    dspN = [(ib, wb, dsp) for ib, wb, dsp in sorted(_LUT4) if dsp  > 0]
    for ib, wb, dsp in dsp0:
        print(f"  LUT4  IB={ib} WB={wb}   DSP=0")
    for ib, wb, dsp in dspN:
        print(f"  LUT4  IB={ib} WB=any  DSP={dsp}")
...
if __name__ == "__main__":
    import argparse
    ap = argparse.ArgumentParser(description="Query or fill conv_layer LUT4/FF models")
    ...
    ap.add_argument("--predict", nargs=5, type=int, metavar=("IC", "OC", "IB", "WB", "DSP"),
                    help="Predict LUT4/FF for IC OC IB WB DSP")
    args = ap.parse_args()
    ...
    if args.predict:
        ...
        try:
            lut4, ff = predict_conv_layer(ic=ic, oc=oc, ib=ib, wb=wb, dsp=dsp)
            print(f"predict(ic={ic}, oc={oc}, ib={ib}, wb={wb}, dsp={dsp})  =>  LUT4={lut4:.0f}  FF={ff:.0f}")
