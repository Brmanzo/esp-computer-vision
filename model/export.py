...
def pack_hex_weights(hex_path: Path,weights_flat: list, weight_bits: int, dsp_count: int, cfg: Any, work_dir: Path, layer_idx: int):
    ...
    print(f"  Generated ROM hex: {filename}")

def export_csv_to_hex(csv_path: Path, sv_path: Path, hex_path: Path, config: ModelConfig):
    ...
    for i, row in enumerate(df.itertuples()):
        ...
        if layer_cfg._dsp_count > 0:
            pack_hex_weights(hex_path, weights, w_bits, layer_cfg._dsp_count, layer_cfg, work_dir, i)
#           (hex_path: Path,weights_flat: list, weight_bits: int, dsp_count: int, cfg: Any, work_dir: Path, layer_idx: int)
