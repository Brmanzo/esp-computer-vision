...
def get_transforms(img_h: int, img_w: int) -> Tuple[Callable, Callable]:
    '''Returns base and augmentation transforms for MNIST.'''
    tfm_base = transforms.Compose([
        transforms.Resize((img_h, img_w)),
        transforms.ToTensor(),
        transforms.Lambda(lambda x: (x > 0.5).float()),  # Binarize to match 1-bit input assumption
    ])

    # Black background (0) matches MNIST digit style
...
def prepare_mnist_data(data_dir: str | Path, img_h: int, img_w: int, batch_size: int) -> Tuple[DataLoader, DataLoader, int]:
    ...
    tfm, _ = get_transforms(img_h, img_w)
